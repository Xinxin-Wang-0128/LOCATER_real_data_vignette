...
# use position info to subset pos and map
...
for(i in 1:nrow(HMM_pars)){
  ...
  # we suggest running LOCATER on putative loci again with the following testing options:
  
  # test.opts <- data.frame( "smt.noise" = c("raw"),
  #                          "thresh" = 0.2,
  #                          "old.sprigs" = c(FALSE),
  #                          "max1var" = c(TRUE),
  #                          "max.k" = 512,
  #                          "sw.thresh" = 6, 
  #       in QForm, we start with Satterthwaite approximation
  #       if approximate -log10pval > sw.thresh, then we do eigen decomposition 
  #                          "eig.thresh" = 6.5,
  #       # if -log10pval < eig.thresh, stop eigen decomposition early
  #                          "calc.obs.T" = TRUE)
